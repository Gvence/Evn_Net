...
if __name__ == '__main__':
    ...
    saver = tf.train.Saver(max_to_keep=4, keep_checkpoint_every_n_hours=0.01)
    ...
    while(True):
        ...
        # 训练
        _, cost, state, pred, testLoss, valLoss = sess.run(
            [model.train_op, model.cost, model.cell_final_state, model.pred, model.testCost, model.valCost],
            feed_dict=feed_dict)
        ...
        if iteration % 1000 == 0:
            ...
            chart_7.plot(np.arange(0, len(valLoss_list)), valLoss_list, 'b-', lw = 2)
            chart_7.plot(np.arange(0, len(trainLoss_list)), trainLoss_list, 'r-', lw = 2)
            plt.pause(0.3)
