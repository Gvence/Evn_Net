...
if __name__ == '__main__':
    ...
    while(True):
        ...
        if iteration%50 == 0 :
            ...
            label_list.append(res.reshape(-1, OUTPUT_SIZE)[-1, :])
            pred_list.append(pred.reshape(-1, OUTPUT_SIZE)[-1, :])
            ...
            vallabel_list.append(valys.reshape(-1, OUTPUT_SIZE)[-1, :])
            valpred_list.append(valpre.reshape(-1, OUTPUT_SIZE)[-1, :])
            # cost保留四位小数输出
        ...
        if iteration % 2500 == 0:
            ...
            output = np.array(pred_list)
            vallabel = np.array(vallabel_list)
            valOutput = np.array(valpred_list)
            plt.clf()
            ...
            chart_1.scatter(np.arange(0, output.shape[0]), output[:, 0], c='red', s=10, marker='x')
            # S_M
            chart_2.scatter(np.arange(0, label.shape[0]), label[:, 1], c='blue', s=10, marker='o')
            chart_2.scatter(np.arange(0, output.shape[0]), output[:, 1], c='red', s=10, marker='x')
            # S_R
            chart_3.scatter(np.arange(0, label.shape[0]), label[:, 2], c='blue', s=10, marker='o')
            chart_3.scatter(np.arange(0, output.shape[0]), output[:, 2], c='red', s=10, marker='x')
            # Dis_1
            chart_4.plot(np.arange(0, label.shape[0]), label[:, 3], 'b-', lw=4)
            chart_4.plot(np.arange(0, output.shape[0]), output[:, 3], 'r--', lw=2)
            # Dis_2
            chart_5.plot(np.arange(0, label.shape[0]), label[:, 4], 'b-', lw=4)
            chart_5.plot(np.arange(0, output.shape[0]), output[:, 4], 'r--', lw=2)
            # Cost
