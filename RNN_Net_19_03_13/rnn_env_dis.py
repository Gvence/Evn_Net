...
def get_batch(trainData):
    ...
    xs = trainData[BATCH_START:BATCH_START + TIME_STEPS*BATCH_SIZE, [3, 4, 5]].reshape((-1,TIME_STEPS ,INPUT_SIZE))
    ys = trainData[BATCH_START + 1:BATCH_START + TIME_STEPS*BATCH_SIZE + 1, [3, 4]].reshape((-1,TIME_STEPS,OUTPUT_SIZE))
...
class LSTMRNN_D(object):
    def __init__(self, n_steps, input_size, output_size, cell_size, batch_size, learning_rate, norm = False):
        ...
        with tf.name_scope('D_inputs'):
            self.xd = tf.placeholder(tf.float32, [None, n_steps, input_size], name='D_xs')
            self.yd = tf.placeholder(tf.float32, [None, n_steps, output_size], name='D_ys')
        with tf.variable_scope('D_in_hidden'):
            self.add_input_layer()
        with tf.variable_scope('D_LSTM_cell'):
            self.add_cell()
        with tf.variable_scope('D_out_hidden'):
            self.add_output_layer()
        with tf.name_scope('D_cost'):
            self.compute_cost()
        with tf.name_scope('D_train'):
            self.train_op = tf.train.AdamOptimizer(self.lr).minimize(self.cost)
            tf.add_to_collection('D_train_op', self.train_op)
    ...
    def add_input_layer(self, ):
        l_in_x = tf.reshape(self.xd, [-1, self.input_size], name='D_2_2D')  # (batch*n_step, in_size)
        # Ws (in_size, cell_size)
        with tf.name_scope('D_input_W'):
            Ws_in = self._weight_variable([self.input_size, self.cell_size])
            tf.summary.histogram('D_input_W', Ws_in)
        # bs (cell_size, )
        with tf.name_scope('D_input_B'):
            bs_in = self._bias_variable([self.cell_size, ])
            tf.summary.histogram('D_input_B', bs_in)
        # l_in_y = (batch * n_steps, cell_size)
        with tf.name_scope('D_Xin_input_W_plus_input_B'):
            # l_in_y = tf.nn.relu(tf.matmul(l_in_x, Ws_in) + bs_in)
            l_in_y = tf.matmul(l_in_x, Ws_in) + bs_in
            tf.summary.histogram('D_input_layer_y',l_in_y)
        # reshape l_in_y ==> (batch, n_steps, cell_size)
        self.l_in_y = tf.reshape(l_in_y, [-1, self.n_steps, self.cell_size], name='D_2_3D')
        if self.norm    :
    ...
    def add_cell(self):
        ...
        tf.summary.histogram('D_rnn_out', self.cell_outputs)

    # 设置 add_output_layer 功能, 添加 output_layer:
    def add_output_layer(self):
        # shape = (batch * steps, cell_size)
        l_out_x = tf.reshape(self.cell_outputs, [-1, self.cell_size], name='D_2_2D')
        with tf.name_scope('D_output_W'):
            Ws_out = self._weight_variable([self.cell_size, self.output_size])
            tf.summary.histogram('D_output_W', Ws_out)
        with tf.name_scope('D_output_B'):
            bs_out = self._bias_variable([self.output_size, ])
            tf.summary.histogram('D_output_B', bs_out)
        # shape = (batch * steps, output_size)
        with tf.name_scope('D_Xout_output_W_plus_output_B'):
            self.pred = tf.matmul(l_out_x, Ws_out) + bs_out
            tf.summary.histogram('D_prediction', self.pred)
            tf.add_to_collection('D_pred', self.pred)

    # RNN 误差计算部分:
    def compute_cost(self):
        losses = tf.contrib.legacy_seq2seq.sequence_loss_by_example(
            [tf.reshape(self.pred, [-1], name='D_reshape_pred')],
            [tf.reshape(self.yd, [-1], name='D_reshape_target')],
        ...
        with tf.name_scope('D_average_cost'):
            self.cost = tf.div(
                tf.reduce_sum(losses, name='D_losses_sum'),
                self.batch_size,
                name='D_average_cost')
            tf.summary.scalar('D_cost', self.cost)
            tf.add_to_collection('D_average_cost', self.cost)
    ...
    def _weight_variable(self, shape, name='D_weights'):
        initializer = tf.random_normal_initializer(mean=0., stddev=1., )
        return tf.get_variable(shape=shape, initializer=initializer, name=name)

    def _bias_variable(self, shape, name='D_biases'):
        initializer = tf.constant_initializer(0.1)
        return tf.get_variable(name=name, shape=shape, initializer=initializer)

def get_VTData(data):
    batch_start = random.randint(0, len(data) - 1 - TIME_STEPS*BATCH_SIZE)
    VTxin = data[batch_start:batch_start + TIME_STEPS*BATCH_SIZE , [3, 4, 5]].reshape((-1,TIME_STEPS ,INPUT_SIZE))
    VTlabel = data[batch_start + 1: batch_start + TIME_STEPS*BATCH_SIZE + 1, [3, 4]].reshape((-1,TIME_STEPS,OUTPUT_SIZE))
...
if __name__ == '__main__':
    ...
    INPUT_SIZE = 3  # 数据输入size
    OUTPUT_SIZE = 2  # 数据输出 size
    ...
    # 搭建 LSTMRNN 模型
    model = LSTMRNN_D(TIME_STEPS, INPUT_SIZE, OUTPUT_SIZE, CELL_SIZE, BATCH_SIZE, LR, norm = False)
    # gpu_options = tf.GPUOptions(per_process_gpu_memory_fraction=0.7)
    # sess = tf.Session(config=tf.ConfigProto(gpu_options=gpu_options))
    sess = tf.Session()
    merged = tf.summary.merge_all()
    writer = tf.summary.FileWriter("logsD", sess.graph)
    saver = tf.train.Saver(max_to_keep=4)
    if os.path.exists('dis_model') and os.path.exists('dis_model/%s.meta'%MODEL_NAME) :
        print('restore last model')
        saver.restore(sess, 'dis_model/%s'%MODEL_NAME)
    else:
    ...
    while(True):
        ...
        if iteration%50 == 0 :
            ...
            valxs = VD[0]
            valys = VD[1]
            valLoss, valpre = sess.run([model.cost, model.pred], feed_dict={model.xd: valxs,model.yd: valys})
            # cost保留四位小数输出
        ...
        if iteration % 2500 == 0:
            # # # 保存模型
            saver.save(sess, 'dis_model/iteration_model', global_step=iteration)
            ...
            chart_2 = fig.add_subplot(7, 1, 2, xlim=(0, vallabel.shape[0]))
            chart_2.set_title('valDis_1')
            chart_3 = fig.add_subplot(7, 1, 3, xlim=(0, vallabel.shape[0]))
            chart_3.set_title('valDis_2')
            ...
            # S_M
            chart_2.plot(np.arange(0, vallabel.shape[0]), vallabel[:, 0], 'b-', lw=4)
            chart_2.plot(np.arange(0, vallabel.shape[0]), valOutput[:, 0], 'r--', lw=2)
            # S_R
            chart_3.plot(np.arange(0, vallabel.shape[0]), vallabel[:, 1], 'b-', lw=4)
            chart_3.plot(np.arange(0, vallabel.shape[0]), valOutput[:, 1], 'r--', lw=2)
            # Dis_1
            chart_4.plot(np.arange(0, label.shape[0]), label[:, 0], 'b-', lw=4)
            chart_4.plot(np.arange(0, label.shape[0]), output[:, 0], 'r--', lw=2)
            # Dis_2
            chart_5.plot(np.arange(0, label.shape[0]), label[:, 1], 'b-', lw=4)
            chart_5.plot(np.arange(0, label.shape[0]), output[:, 1], 'r--', lw=2)
            # Cost
