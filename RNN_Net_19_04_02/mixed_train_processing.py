...
def get_batch(trainData):
    ...
    # 重新选取片段或者起始数据达到临界值时重新选取起始值
    while  INITSTATE:
        BATCH_START = np.array([])
        # batch_start从数据中循环抽取
        for i in range(BATCH_SIZE):
            #每次抽取的start存入list
            BATCH_START = np.append(BATCH_START, random.randint(0, len(trainData) - 1 - 2 * TIME_STEPS - BATCH_SIZE))
        #关闭batch初始化
        INITSTATE = False
    ...
    ys = np.array([])
    #依次读取每一个起点
    for j in BATCH_START:
        j = int(j)
        xs = np.append(xs,trainData[j:j + TIME_STEPS, :])
        ys = np.append(ys,trainData[j + 1:j + TIME_STEPS + 1, :])
    return [xs.reshape((-1,TIME_STEPS ,INPUT_SIZE)), ys.reshape((-1,TIME_STEPS,INPUT_SIZE))]
...
if __name__ == '__main__':
    ...
    TIME_STEPS =5 # backpropagation through time 的time_steps 误差反传的步长
    BATCH_SIZE = 100
    STEP_LENGTH = 4 #每次训练的步长的数量
    ...
    while(True):
        ...
        if times_tip % STEP_LENGTH == 0:
            ...
            S = np.concatenate((S[:,1:, :],np.concatenate((car_S, action.reshape((BATCH_SIZE, -1))), axis=1).reshape((BATCH_SIZE, -1, INPUT_SIZE))),axis=1)
            feed_dict = {
                model.xs: S,
                model.ys: S_[:,:,:-1],
                model.D_cell_init_state: D_state,  # 保持 state 的连续性
        ...
        if S_cost <0.1 and  D_cost < 1000 :
            car_S = np.concatenate((np.round(S_pred[:,-1, :],0), D_pred[:,-1, :]))
            right_times += 1
        else:
            #right_times = 0
            car_S = S_[:,-1,:-1]
        ...
        if times_tip%(BATCH_SIZE*50 ) == 0 :
            ...
            while times < STEP_LENGTH:
                if INITSTATE:
                    val_right_times = 0
                    val_S, val_S_ = get_batch(valData)
                    val_feed_dict = {model.xs: val_S, model.ys:val_S_[:,:,:-1]}
                else:
                    action = val_S_[:, -1, -1]
                    val_S_ = get_batch(valData)[1]
                    val_S = np.concatenate((val_S[:,1:,:],np.concatenate((val_car_S, action.reshape((BATCH_SIZE, -1))), axis=1).reshape((BATCH_SIZE,-1, INPUT_SIZE))), axis=1)
                    feed_dict = {
                        model.xs: val_S,
                        model.ys: val_S_[:,:,:-1]
                ...
                if val_D_cost < 1000 and val_S_cost < 0.1 :
                    val_car_S = np.concatenate((np.round(val_S_pred[:,-1, :],0), val_D_pred[:,-1, :]))
                    val_right_times += 1
                else:
                    #val_right_times = 0
                    val_car_S = val_S_[:,-1,:-1]
            ...
            # cost保留四位小数输出
            print('EPO:',int((iteration*BATCH_SIZE*(STEP_LENGTH+TIME_STEPS))/(int(DATA_LONG/BATCH_SIZE)*BATCH_SIZE)),'ITER: ', iteration, 'D_train_cost: ', D_mean_cost, 'S_train_cost:', round(S_mean_cost, 4),
                  'Accuraty:%.2f%%\n'%(100*right_times/STEP_LENGTH),
                  '\t\t\t\t','D_val_cost: ', round(val_D_mean_cost, 4), 'S_val_cost:', round(val_S_mean_cost, 4),
                  'Val_Accuraty:%.2f%%\n'%(100*val_right_times/STEP_LENGTH)
                  )
